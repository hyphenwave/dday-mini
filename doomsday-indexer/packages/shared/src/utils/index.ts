...
export function calculateRoundDuration(
  ...
  // Round 1: initialDays (e.g., 7d). Then decrease by 1 day per round
  // until we drop below 1 day, after which we fix at minHours (e.g., 8h).
  if (roundNumber === 1) {
    return initialDays * 24 * 60 * 60
  }

  const daysRemaining = initialDays - (roundNumber - 1) * decreasePerRound
  if (daysRemaining >= 1) {
    return daysRemaining * 24 * 60 * 60
  }
